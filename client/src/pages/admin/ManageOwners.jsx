...
const ManageOwners = () => {
    ...
    const fetchOwners = async () => {
        try {
            ...
            console.log('Admin owners response:', data)
            if (data.success) {
            ...
            setError(error.message || 'Error fetching owners')
    ...
    const deleteOwner = async (ownerId) => {
        try {
            const confirm = window.confirm("Are you sure you want to delete this owner and all their cars?")
    ...
    return (
        <div className='px-4 pt-10 md:px-10 w-full'>
            ...
            <Title title="Manage Owners" subTitle="View and manage all car owners" />
                        ...
                        <tbody>
                            {owners.map((owner, index) => (
                                <tr key={index} className='border-t border-gray-300 hover:bg-gray-50'>
                                    ...
                                    <td className='p-3'>
                                        <span className='px-3 py-1 rounded-full text-xs bg-green-100 text-green-500'>
                                            {owner.role}
                                        </span>
                                    </td>
                                    <td className='p-3'>
