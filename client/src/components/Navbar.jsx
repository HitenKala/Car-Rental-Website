...
const Navbar = ({ setShowLogin }) => {
  ...
  return (
    ...
    ${location.pathname === "/" && "bg-[#cdc7cc] border-b-3 border-[#ff344c]"}`}>
        ...
        <Link to={"/"}>
            <img src={assets.turbo_rides2} type='png' alt="logo" className="h-22" />
        </Link>
        ...
        <div className={`max-sm:fixed max-sm:h-screen max-sm:w-full max-sm:top-18   
            ...
            ${open ? "max-sm:translate-x-0" : "max-sm:-translate-x-full"}`}>
                ...
                <div className='hidden lg:flex items-center text-sm gap-2 border border-gray-500 px-3 rounded-full max-w-56'>
                ...
                <div className='flex max-sm:flex-col items-start sm:items-center gap-6'>
                    ...
                    <button onClick={()=>setShowLogin(true) } className='cursor-pointer px-8 py-2 bg-gray-100 hover:bg-[#232323] transition-all
                     text-[#DA2917] hover:text-[#fff] rounded-lg font-bold '>Login</button>
                </div>
